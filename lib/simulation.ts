export type CircuitKey = 'escape' | 'heading' | 'motion';
export type ModelKey = 'lif' | 'hh' | 'graded';
export type Wiring = 'recorded' | 'rewired' | 'shuffled';
export interface Neuron {
  id: number;
  type: string;
  label: string;
  nt: string;
  ntConfidence: number | null;
...
export function prepareEdges(c: Circuit, p: Settings) {
  ...
  if (p.wiring === 'rewired') {
    ...
    for (let i = 0; i < e.length * 10; i++) {
      const a = Math.floor(rng() * e.length),
        b = Math.floor(rng() * e.length);
      if (a === b) continue;
      const x = e[a],
        y = e[b];
      if (
  ...
  if (p.dropout > 0) e = e.filter(() => rng() >= p.dropout);
  return e;
}
export function sign(nt: string) {
...
const vtrap = (x: number) =>
  Math.abs(x) < 1e-5 ? 1 + x / 2 : x / -Math.expm1(-x);
export function rates(v: number) {
  return [
    vtrap((v + 40) / 10),
    4 * Math.exp(-(v + 65) / 18),
...
export function hhStep(
  ...
  h = gate(h, r[2], r[3]);
  n = gate(n, r[4], r[5]);
  const na = 120 * m * m * m * h,
    k = 36 * n * n * n * n,
    g = na + k + 0.3,
    eq = (na * 50 + k * -77 + 0.3 * -54.4 + current) / g;
...
function stimulus(
  ...
  if (key === 'escape') {
    if (node.type === 'LC4')
      return (
        Math.min(1, (t - 100) / 180) * Math.exp(-Math.max(0, t - 300) / 45)
      );
    if (node.type === 'LPLC2')
      return (
        (1 / (1 + Math.exp(-(t - 240) / 28))) *
        Math.exp(-Math.max(0, t - 360) / 25)
      );
    return 0;
  }
  if (key === 'heading') {
    if (node.type !== 'EPG' || t > 380) return 0;
    ...
    const delta = Math.atan2(
      Math.sin(((column - center) * Math.PI) / 8),
      Math.cos(((column - center) * Math.PI) / 8),
    );
    return Math.exp((-delta * delta) / 0.35);
  }
  if (/^T[45]/.test(node.type)) return 0;
  const x = node.position?.[0] ?? 400;
  const center = 270 + direction * Math.max(-70, Math.min(70, (x - 440) * 1.5));
  return Math.exp(-(((t - center) / 35) ** 2));
}
export function trajectory(voltage: number[][]) {
  const n = voltage.length,
    T = voltage[0]?.length ?? 0;
  if (!n || !T) return { points: [], explained: [] };
  ...
  for (let i = 0; i < n; i++)
    for (let j = 0; j <= i; j++) {
      let s = 0;
      for (let t = 0; t < T; t++) s += (X[i][t] * X[j][t]) / T;
      C[i][j] = C[j][i] = s;
      if (i === j) trace += s;
    }
  ...
  for (let k = 0; k < 2; k++) {
    ...
    for (let iter = 0; iter < 32; iter++) {
      ...
      for (const prev of vectors) {
        ...
        for (let j = 0; j < n; j++) out[j] -= dot * prev[j];
      }
    ...
    values.push(
      v.reduce(
        (s, x, i) => s + x * C[i].reduce((q, y, j) => q + y * v[j], 0),
        0,
      ),
    );
  }
  const points = Array.from({ length: T }, (_, t) =>
    vectors.map((v) => v.reduce((s, x, i) => s + x * X[i][t], 0)),
  );
...
export function simulate(
  ...
  const n = c.nodes.length,
    dt = customDt ?? (model === 'hh' ? 0.025 : 0.1),
    steps = Math.round(duration / dt),
    every = Math.round(1 / dt),
    rng = random(p.seed),
    e = prepareEdges(c, p),
  ...
  const voltage: number[][] = Array.from({ length: n }, () => []),
    spikes: number[][] = Array.from({ length: n }, () => []),
    time: number[] = [],
    stim: number[] = [];
  const rawVoltage = Array.from({ length: n }, () => [] as number[]);
  const diagnosticRows = () => Array.from({ length: n }, () => [] as number[]);
  const diagnostics = {
  ...
  for (let step = 0; step <= steps; step++) {
    ...
    if (step % every === 0) {
      net.fill(0);
      for (const [a, b, w] of edges) net[b] += w * syn[a];
      ...
      for (let i = 0; i < n; i++) {
        ...
        const q = net[i] * p.gain,
          V = v[i];
        let external: number, synaptic: number, intrinsic: number;
        if (model === 'lif') {
          external = (24 * u) / 20;
          synaptic = (35 * q) / 20;
          intrinsic = (-65 - V) / 20;
        ...
        diagnostics.derivative[i].push(
          held ? 0 : external + synaptic + intrinsic,
        );
    ...
    for (let i = 0; i < n; i++) {
      ...
      const input = stimulus(key, c.nodes[i], i, n, t, p.direction) * p.drive,
        drive = net[i] * p.gain,
        old = v[i];
      let spike = false;
      syn[i] *= Math.exp(-dt / 8);
      if (model === 'lif') {
        ...
        const ex = Math.max(0, drive) * 0.05 + input * 0.065,
          inh = Math.max(0, -drive) * 0.05,
          conductance = 0.05 + ex + inh,
          eq = (-65 * 0.05 - 75 * inh) / conductance;
